...
@MainActor extension AppModel {
    func loadDemo() {
        ...
        for i in 1...3 {
            ...
            state.inventory = HardwareInventory(hostname: state.profile.name, devices: [
                HardwareDevice(id: "cpu", kind: .cpu, name: "CPU", model: "NVIDIA Grace · GB10", defaultVisible: true, metadata: ["cores": "20", "architecture": "aarch64"]),
                HardwareDevice(id: "memory", kind: .memory, name: "Memory", model: "LPDDR5X · Unified memory", defaultVisible: true, metadata: [:]),
                HardwareDevice(id: "disk:demo", kind: .disk, name: "0 (NVMe)", model: "4 TB NVMe SSD", defaultVisible: true, metadata: [:]),
                HardwareDevice(id: "net:demo", kind: .network, name: "ethernet0", model: "Ethernet", defaultVisible: true, metadata: ["rdma_ports": "[]", "address": "192.0.2.10"]),
    ...
    func demoSnapshot(time: Double, offset: Int) -> MetricsSnapshot {
        ...
        return MetricsSnapshot(timestamp: time, uptime: 106393, devices: [
            "cpu": DeviceMetrics(values: ["usage": busy, "user": busy - 8, "system": 8, "frequency": 3280, "temperature": 68.5, "load": 7.8],
                                 cores: cores,
                                 sensors: ["TS0E": 56.2, "TS0P": 67.1, "TS1E": 57.5, "TS1P": 68.5]),
            "memory": DeviceMetrics(values: ["usage": 58, "used": 70.5 * gib, "total": 121.6 * gib, "available": 51.1 * gib, "cached": 12.4 * gib, "swapUsed": 0, "swapTotal": 16 * gib]),
            "disk:demo": DeviceMetrics(values: ["usage": busy / 3, "read": busy * 1_000_000, "write": 3_000_000, "temperature": 44, "capacity": 4_000_000_000_000, "total": 3.75e12, "free": 2.5e12]),
            ...
            "gpu:demo": DeviceMetrics(values: ["usage": busy + 18, "temperature": 58, "power": 38.4, "frequency": 2483, "memoryActivity": 12])
        ])
    }
}
