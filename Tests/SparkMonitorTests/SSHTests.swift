...
struct SSHTests {
    @Test @MainActor func realSSHPasswordKeyAndTrust() async throws {
        ...
        let password = SSHPerformanceCollector()
        ...
        for (secret, accept) in [("wrong-password", true), ("test-only-secret", false)] {
            let invalid = SSHPerformanceCollector()
            do {
            ...
            } catch { #expect(!SSHPerformanceCollector.isNetworkError(error)) }
            await invalid.stop()
        }
        profile.authentication = "key"; profile.keyPath = keyPath
        let key = SSHPerformanceCollector(); received = false
    ...
    @MainActor func remoteSparkWhenConfigured() async throws {
        ...
        let profile = HostProfile(name: "Integration", host: host, username: user, keyPath: env["SPARK_TEST_KEY"] ?? "~/.ssh/id_ed25519", platform: env["SPARK_TEST_PLATFORM"])
        ...
        try await collector.start(profile: profile, secret: nil, trust: { $0 == fingerprint }, receive: { msg in
            if let pid = msg.samplerPid { samplerPid = pid }
        ...
        #expect(sampleCount == 2); #expect(coreCount == Int(env["SPARK_TEST_CORES"] ?? "20"))
        if let samplerPid { print("Sampler PID for shutdown verification: \(samplerPid)") }
    }
}
